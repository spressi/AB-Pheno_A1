...
# - check for plausibility after subject is finished (may interpolate missing beats on the end of a segment)
#
# Extensions:
...
#input
if (exists("path.physio")==F)
  path.physio = "." #TODO enter input path here if files are not in project root
if (path.physio %>% endsWith("/") == F & path.physio != ".") path.physio = "/" %>% paste0(path.physio, .)
...
if (exists("path.rpeaks")==F)
  path.rpeaks = "rpeaks/" #output path
...
path.screenshots = "plots HR/" %>% paste0(path.rpeaks, "../", .) #paste0(ifelse(exists("path.rds"), path.rds, path.physio), .)
...
manualcheck = function(t_trial, ecg_trial, rpeak) {
  ...
  while (!ready) {
    ...
    while (!ok) {	
      ...
      if (length(rpeak)>0) {
        ...
        lines(rpeak[2:length(rpeak)],hr,col=c("black","red")[problem+1],lwd=2)
      } else {
      ...
      if (is.null(location) | (length(location$x)==1)) {
        ...
        rpeak = rpeak[!(rpeak>min(location$x) & rpeak<max(location$x))]
      }
    }
    ...
    while (!ok) {	
      ...
      if (is.null(location)) {
        ...
        if (inrect(location$x, location$y, min(t_trial)-2, min(hr), min(t_trial), max(hr))) {
          ...
          rpeak = c(rpeak,t_trial[critsys[ecg_trial[critsys]==max(ecg_trial[critsys])][1]])
          rpeak = sort(rpeak)
          # Delete duplicates
          difs = diff(rpeak)
          rpeak = rpeak[c(TRUE,difs!=0)]
        }
      }
    }
  }
...
subjects.ecg = list.files(path.physio, pattern=ifelse(unzipFiles, ".zip", ".txt"), full.names=TRUE)
...
while(T) {
  ...
  # - timeline
  # - marker (markertime)
  if (unzipFiles) code %>% paste0(".zip") %>% unzip(exdir=path.physio) #only works in Windows?
          ...
          if (substring(r.threshold_new, 1, 1)=="m") {   # Manual editing
            ...
            for (p in hrModProblems) {
              if (p-1 <= 0 || p+1 > length(hrtrial)) 
                print(paste0("Cannot interpolate on the border of a segment. Problem will be solved when finishing the subject."))
  ...
  while (length(hrModProblems) > 0 && interpolate) {
    ...
    allrpeak[hrModProblems] %>% sapply(function(x) return({x > analysis.sequence} %>% which() %>% tail(1))) %>% 
      paste0("Segment ", ., " (of ", length(analysis.sequence), ")") %>% print()
